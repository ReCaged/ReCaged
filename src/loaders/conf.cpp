...
{
	Log_printf(1, "Loading conf file: %s", name);
	...
	while (file.Read_Line())
	{
		...
		{
			Log_printf(0, "WARNING: parameter \"%s\" does not match any parameter name!", file.words[0]);
			continue;
		}
		...
		{
			Log_printf(0, "WARNING: parameter \"%s\" has wrong ammount of args: expected: %i, got %i!",file.words[0], index[i].length, file.word_count-1);
			continue;
		}
			...
			switch (index[i].type)
			{
				...
				case 'f':
					...
					if (strleft == file.words[argnr+1])
						Log_printf(0, "WARNING: \"%s\" is invalid for \"floating point\" parameter \"%s\"!", file.words[argnr+1], index[i].name);
					else //ok
				...
				case 'd':
					...
					if (strleft == file.words[argnr+1])
						Log_printf(0, "WARNING: \"%s\" is invalid for \"double precision floating point\" parameter \"%s\"!", file.words[argnr+1], index[i].name);
					else //ok
				...
				case 'R':
					...
					if (strleft == file.words[argnr+1])
						Log_printf(0, "WARNING: \"%s\" is invalid for \"dReal floating point\" parameter \"%s\"!", file.words[argnr+1], index[i].name);
					else
				...
				case 'b':
					...
					//false if: "false", "off", "no", "0"
						...
						*(((bool*)(memory+index[i].offset))+argnr) = false;
						...
						Log_printf(0, "WARNING: \"%s\" is invalid for \"boolean\" parameter \"%s\"!", file.words[argnr+1], index[i].name);
				break;
				...
				case 'i':
					...
					if (strleft == file.words[argnr+1])
						Log_printf(0, "WARNING: \"%s\" is invalid for \"integer\" parameter \"%s\"!", file.words[argnr+1], index[i].name);
					else
				...
				case 's':
					...
					if (strlen(file.words[argnr+1]) >= Conf_String_Size) //equal or bigger than max size
						Log_printf(0, "WARNING: word in conf file was too big for direct string copy!");
					else //ok, just copy
				...
				//unknown
				default:
					Log_printf(0, "WARNING: parameter \"%s\" is of unknown type (\"%c\")!", file.words[0], index[i].type);
				break;
			}
		}
