...
int lua_log_add (lua_State *lua)
{
	...
	if (n >= 2 && lua_isnumber(lua, 1))
	{
		...
		if (logfile || level <= stdout_verbosity)
		{
			...
			//loop through lua args, add each string to buffer
			int arg = 2;
			size_t l, pos=3;
			const char *str;
			while (--n)
			{
				str = lua_tostring(lua, arg++);
				l = strlen(str);

//TODO: like this, like this: int i=vsnprintf (logbuffer+3, LOG_BUFFER_SIZE-3, text, list);
				if ( (pos+l+1) > LOG_BUFFER_SIZE)
				{
					puts("ERROR DURING LOG OUTPUT GENERATION: text buffer too small!");
					SDL_mutexV(logmutex);
					return 0;
				}

				strcpy(logbuffer+3, str);
			}
